...
for cat in attack_cats:
    ...
    if cat == 'normal':
        ...
        nn_normal_inds = indices[:len_of_subset][:max_training_samples]  #0
...
def plot_feature_importances(X_train, importances, indices, std, title, label):
    plt.figure()
    plt.title(title)
...
plot_feature_importances(X_rf_train, rf_importances, rf_indices, rf_std, title='Feature importances (Random forest)', label='rf')
...
plot_feature_importances(X_nn_train,
                        nn_importances, nn_indices, nn_std, title='Feature importances (Neural network)', label='nn')

NB_RF_FEATURES = 10
NB_NN_FEATURES = 25#25
...
import h5py
h5f = h5py.File('datasets_25.h5', 'w')
